;; Tag 0: Call at 100:28: 100:31
;; Tag 1: Call at 101:28: 101:31
;; Tag 2: Call at 101:36: 101:39
;; Tag 3: Call at 101:9: 101:40
;; Tag 4: Call at 102:17: 102:20

(qualif EqZero ((v int)) ((= v 0)))
(qualif GtZero ((v int)) ((> v 0)))
(qualif GeZero ((v int)) ((>= v 0)))
(qualif LtZero ((v int)) ((< v 0)))
(qualif LeZero ((v int)) ((<= v 0)))
(qualif Eq ((a int) (b int)) ((= a b)))
(qualif Gt ((a int) (b int)) ((> a b)))
(qualif Ge ((a int) (b int)) ((>= a b)))
(qualif Lt ((a int) (b int)) ((< a b)))
(qualif Le ((a int) (b int)) ((<= a b)))
(qualif Le1 ((a int) (b int)) ((<= a (- b 1))))
(constant gt (func 1 (@(0) @(0) ) bool))
(constant ge (func 1 (@(0) @(0) ) bool))
(constant lt (func 1 (@(0) @(0) ) bool))
(constant le (func 1 (@(0) @(0) ) bool))
(var $k0 (int int int int)) ;; orig: $k3
(var $k1 (int int int int)) ;; orig: $k4
(var $k2 (int int int int)) ;; orig: $k0
(var $k3 (int int int int int)) ;; orig: $k1
(var $k4 (int int int int int)) ;; orig: $k2
(var $k5 (int int int int int)) ;; orig: $k5
(var $k6 (int int int int int)) ;; orig: $k6
(var $k7 (int int int int int int int)) ;; orig: $k7
(var $k8 (int int int int int int int)) ;; orig: $k8
(var $k9 (int int int int int int int int int)) ;; orig: $k9

(constraint
 (forall ((a0 int) (true))
  (forall ((_$ int) ((>= a0 0)))
   (forall ((a1 int) (true))
    (forall ((_$ int) ((<= 0 a1)))
     (forall ((a2 int) (true))
      (forall ((_$ int) ((<= 0 a2)))
       (and
        (forall ((a3 int) (true))
         ($k0 a3 a0 a1 a2))
        (forall ((_$ int) ((>= a1 0)))
         (forall ((_$ int) ((<= 0 a1)))
          (and
           (forall ((a4 int) ((= a4 0)))
            ($k1 a4 a0 a1 a2))
           (forall ((_$ int) ((<= 0 a1)))
            (and
             (forall ((a5 int) ((= a5 0)))
              ($k2 a5 a0 a1 a2))
             (forall ((a6 int) (true))
              (forall ((_$ int) ((= a6 a0)))
               (forall ((a7 int) ((= a7 0)))
                ($k3 a6 a7 a0 a1 a2))))
             (forall ((a8 int) (true))
              (forall ((_$ int) ($k1 a8 a0 a1 a2))
               (forall ((a9 int) ((= a9 0)))
                ($k4 a8 a9 a0 a1 a2))))
             (forall ((a10 int) (true))
              (forall ((_$ int) ($k2 a10 a0 a1 a2))
               (and
                (forall ((a11 int) (true))
                 ($k5 a11 a0 a1 a2 a10))
                (forall ((_$ int) ((>= a2 0)))
                 (forall ((_$ int) ((< a10 a2)))
                  (and
                   (forall ((a12 int) (true))
                    ($k6 a12 a0 a1 a2 a10))
                   (tag ((< a10 a2)) "0")
                   (forall ((a13 int) (true))
                    (forall ((_$ int) ($k6 a13 a0 a1 a2 a10))
                     (forall ((_$ int) ((<= 0 a13)))
                      (forall ((a14 int) (true))
                       (forall ((_$ int) ((>= a14 0)))
                        (and
                         (forall ((a15 int) (true))
                          (forall ((_$ int) ($k3 a15 a10 a0 a1 a2))
                           ($k7 a15 a0 a1 a2 a10 a13 a14)))
                         (tag ((< a14 a1)) "1")
                         (forall ((a16 int) (true))
                          ($k8 a16 a0 a1 a2 a10 a13 a14))
                         (tag ((< a10 a2)) "2")
                         (forall ((a17 int) (true))
                          (forall ((_$ int) ($k8 a17 a0 a1 a2 a10 a13 a14))
                           (forall ((_$ int) ((<= 0 a17)))
                            (forall ((a18 int) (true))
                             (forall ((_$ int) ($k7 a18 a0 a1 a2 a10 a13 a14))
                              (and
                               (tag ((= a17 a18)) "3")
                               (forall ((a19 int) (true))
                                (forall ((_$ int) ($k4 a19 a10 a0 a1 a2))
                                 ($k9 a19 a0 a1 a2 a10 a13 a14 a17 a18)))
                               (tag ((< a14 a1)) "4")
                               (forall ((a20 int) (true))
                                (forall ((_$ int) ((>= a20 0)))
                                 (forall ((_$ int) ($k9 a20 a0 a1 a2 a10 a13 a14 a17 a18))
                                  (and
                                   (forall ((a21 int) ((= a21 (+ a20 1))))
                                    ($k9 a21 a0 a1 a2 a10 a13 a14 a17 a18))
                                   (forall ((a22 int) ((= a22 (+ a10 1))))
                                    ($k2 a22 a0 a1 a2))
                                   (forall ((a23 int) (true))
                                    (forall ((_$ int) ($k7 a23 a0 a1 a2 a10 a13 a14))
                                     (forall ((a24 int) ((= a24 (+ a10 1))))
                                      ($k3 a23 a24 a0 a1 a2))))
                                   (forall ((a25 int) (true))
                                    (forall ((_$ int) ($k9 a25 a0 a1 a2 a10 a13 a14 a17 a18))
                                     (forall ((a26 int) ((= a26 (+ a10 1))))
                                      ($k4 a25 a26 a0 a1 a2)))))))))))))))))))))))))))))))))))))))

